...
export default {
  ...
  methods: {
    ...
    load_cep_conjuge() {
      // const url_cep;
      this.form.conjuge.cep = this.form.conjuge.cep.trim().replace(/[^0-9]/g, '');
      // this.cep = 'https://viacep.com.br/ws/' + this.cep + '/json';
      // clear all headers axios to viacep
      // axios.defaults.headers.common = null;
      axios.get(`https://cors-anywhere.herokuapp.com/https://viacep.com.br/ws/${this.form.conjuge.cep}/json`).then((response) => {
        this.form.conjuge.rua = response.data.logradouro;
        this.form.conjuge.bairro = response.data.bairro;
        this.form.conjuge.uf = response.data.uf;
        this.form.conjuge.cidade = response.data.localidade;
      }).catch((error) => {
        console.log(error.statusText);
      });
    },
    ...
    async submit() {
      ...
      // api.post(`${this.$url}/cliente`, JSON.stringify(form))
      //   .then(async ({ data }) => {
      //     this.feedback = true;
    ...
    >
      ...
      >
        ...
        >
         ...
         <!-- Profissão e Nacionalidade Cônjuge -->
          <div class="create-user__form-item is-multi">
            <int-input
              label="Profissão"
              v-model="form.conjuge.profissao"
            ...
            <int-input
              label="Nacionalidade"
              v-model="form.conjuge.nacionalidade"
          ...
          >
            <int-input
              label="Complemento"
              v-model="form.conjuge.complemento"
