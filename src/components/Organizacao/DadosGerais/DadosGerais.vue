<template>
<div class="espaco">
    <el-form size="small" ref="dadosGeraisForm" :label-position="labelPosition" :rules="rules" :model="dadosGeraisForm" label-width="100px">


        <el-row >
        <el-col :span="20">
            <el-form-item label="Nome" prop="nome">
                <el-input v-model="dadosGeraisForm.nome" name="nome"></el-input>
                </el-form-item>
        </el-col>
        </el-row>
...
<el-row type="flex" class="row-bg" justify="end">
  <el-button size="small" type="success" @click="Adicionar('dadosGeraisForm')">Salvar</el-button>
  ...
  methods: {
      Adicionar(dadosGeraisForm) {
       ...
       this.$refs[dadosGeraisForm].validate((valid) => {
          if (valid) {
            this.$store.dispatch('adicionarDadosGerais', _dadosGerais)
            .then(() => {             
              this.$message({
                type: 'success',
                message: 'Dados Gerais cadastrado.'
              });

               this.resetForm(dadosGeraisForm)
      ...
      resetForm(dadosGeraisForm) {
        this.$refs[dadosGeraisForm].resetFields();
      }
    }
}
</script>
